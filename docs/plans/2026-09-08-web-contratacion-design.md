...
## Integración autorizada en develop

El 2026-09-08 se autoriza subir esta web a `develop` y retirar el posicionamiento sobre historia clínica. Se elimina el anuncio condicional de la portada y la referencia restante de metadatos. Los textos legales permanecen completos, con `noindex, follow` y fuera del sitemap. La integración preserva el portal de médicos, administración, profesionales y previews que existían solo en `develop`; estos últimos y las rutas de gestión se excluyen de buscadores. El permiso alcanza a `develop`, sin publicar en `main` ni enviar comunicaciones.
